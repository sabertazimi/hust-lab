...
int fg_job_stopped_or_finished;
...
void eval(char *cmdline) 
{
    ...
    if (!builtin_cmd(argv)) {
        ...
        if ((pid = Fork()) == 0) {
            ...
            Setpgid(0, 0);
            Execve(argv[0], argv, environ);         // cuz unblock, it can delivery signal
        }
        ...
        Sigprocmask(SIG_UNBLOCK, &mask, NULL);     // til now, it can handle signal
